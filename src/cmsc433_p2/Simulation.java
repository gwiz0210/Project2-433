...
public class Simulation {
	...
	static int cookAvail;
	static int capacity;
	static int cookNum;
	static int custNum;
	static int machineLoad;
	public static HashMap<Food, Machine> machines = new HashMap<Food, Machine>();
	public static HashMap<Integer, List<Food>> orders = new HashMap<Integer, List<Food>>();
	public static LinkedList<Integer> orderNums = new LinkedList<Integer>();
	public static HashMap<Integer, List<Food>> ordersDone = new HashMap<Integer, List<Food>>();
	...
	public static synchronized void incrementCooks(){
		cookAvail++;
	}
	
	public static synchronized void decrementCooks(){
		cookAvail--;
	}
	...
	public static List<SimulationEvent> runSimulation(
		...
		// Start up machines
		machines.put(FoodType.burger, new Machine("Grill", FoodType.burger, machineCapacity));
		machines.put(FoodType.fries, new Machine("Fryer", FoodType.fries, machineCapacity));
		machines.put(FoodType.coffee, new Machine("CoffeeMaker2000", FoodType.coffee, machineCapacity));
		...
		// Shut down machines
		logEvent(SimulationEvent.machineEnding(machines.get(FoodType.burger)));
		machines.remove(FoodType.burger);
		logEvent(SimulationEvent.machineEnding(machines.get(FoodType.fries)));
		machines.remove(FoodType.fries);
		logEvent(SimulationEvent.machineEnding(machines.get(FoodType.coffee)));
		machines.remove(FoodType.coffee);
		machines = null;
	...
	public static void main(String args[]) throws InterruptedException {
		...
		cookNum = numCooks;
		cookAvail = numCooks;
		custNum = numCustomers;
		machineLoad = machineCapacity;
